...
local manager = {
    ...
    toRun = {},
    online = {
        server = 'https://127.0.0.1:1234/api/v1/' --no, server is not ready yet
    },
    TPTMPSupport = true,
                        ...
                        buttons = {
                            {
                                ...
                                action = function()
                                    platform.openLink("https://github.com/BomberPlayz/voxelman/")
                                end
                            },
                            {
                                ...
                                action = function()
                                    ...
                                    if ok==true then
                                        ...
                                        manager.pushNotification{
                                            text = 'Update failed; error code '..err.."; Is your internet having issues?"
                                        }
                                    end
                                end
                            },
...
manager.pushNotification{
    text = "We recommend you to download Vernelly, BomberPlayz's manager.",
    life = 800,
    fade = true,
    fadeStart = 15,
    backgroundColor = {240,40,40,200},
    borderColor = {240,40,40,200},
    buttons={
        {
            text="Install vernelly",
            action=function()
                platform.openLink("https://github.com/BomberPlayz/Vernelly")
            end
        },
        {
            text="Vernelly Github",
            action=function()
                platform.openLink("https://github.com/BomberPlayz/Vernelly")
            end
        }
...
manager.pushNotification{
    text = "No more support, updates, security updates will be available.",
    life = 800,
    fade = true,
    fadeStart = 15,
    backgroundColor = {240,40,40,200},
...
manager.pushNotification{
    text = "Voxel has discontinued VoxelMan, which means:",
    life = 800,
    fade = true,
    fadeStart = 15,
    backgroundColor = {240,40,40,200},
...
manager.pushNotification{
    text = "VoxelMan is now End Of Life.",
    life = 800,
    fade = true,
    fadeStart = 15,
    backgroundColor = {240,40,40,200},
...
_G.voxelman = {
    expose = function(t)
        ...
        local con = tpt.confirm('VOXELMAN','A script is requesting access to the internal table.\nThis will give the script (almost) full control over the manager.\nThis is not recommended, as it is a big security risk! ')
        if con then
            return manager
        else
            return false
